...
const Home = ({ navigation, route }) => {
  ...
  const [currentTextIndex, setCurrentTextIndex] = useState(count);
  ...
  useEffect(() => {
    if (texts.length && count >= texts.length) {
      console.log(`Redirecting because count (${count}) is >= texts.length (${texts.length})`);
      Alert.alert("you have recorded all the texts, you can now only view your recordings.");
 ...
 useEffect(() => {
  ...
  Alert.alert(
    "Instructions",
    "Press the mic icon to start recording. Press mic icon again to stop recording. " +
    "You can also record again by pressing record again button " +
    "or move to the next recording and upload the current recording by pressing the next recording button. " +
    "Read the text that appears.",
    [
      { text: "OK", onPress: () => setshowtext(true) }
    ],
    { cancelable: false }
  );
}, []);
...
const uploadRecording = async (uri) => {
    ...
    const fileName = `recording_${Date.now()}_${name}_TextID_${texts[currentTextIndex]._id}.3gp`;
    ...
    try {
        const response = await axios.post('https://shoaib-app-75a4a2e9e1e5.herokuapp.com/recordings/createRecordings', formData,{
          headers: {
...
const fetchTexts = async () => {
  try {
    const response = await axios.get('https://shoaib-app-75a4a2e9e1e5.herokuapp.com/text/getall');
    console.log(response.data.message); // Log to verify
