...
let registerUser = async (req, res) => {
    ...
    User.findOne({ email: req.body.email })
        .then((existingUser) => {
            ...
            newUser.save()
                .then(() => {
                    res.status(200).send({ message: "User registered successfully!", userId: savedUser._id });
                })
                .catch((err) => {
...
let revisitUser = async (req, res) => {
    ...
    .then((existingUser)=>{
        if(existingUser)
        {
            res.status(200).send({message:"user found",userId: existingUser._id,count: existingUser.count});
        }
        else
        {
