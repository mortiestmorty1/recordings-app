...
let getallrecsofuser = async (req, res) => {
    ...
    // Assuming 'email' is a field in your Recording model that references the user
    Recording.find({ userId: req.query.userId})
        .then(recordings => {
            if(recordings.length > 0) {
                // Recordings found, send them back
                res.status(200).send(recordings); // This will send an array of recordings
            } else {
