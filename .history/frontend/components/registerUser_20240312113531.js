...
const RegisterUser = ({ onSubmit,navigation }) => {
  const [formData, setFormData] = useState({
   ...
   const handleRegistration = () => {
    try {
      const response = await axios.post('http://localhost:3001/user/RegisterUser', formData);
  ...
  >
    <ScrollView contentContainerStyle={styles.container}>
      ...
      <TextInput
        style={styles.input}
        onChangeText={setFormData.name}
        value={formData.name}
      />
      <Text style={styles.liltitles}>Email:</Text>
      <TextInput
        style={styles.input}
        onChangeText={setFormData.email}
        value={formData.email}
      />
      <Text style={styles.consent}>Consent:</Text>
      <Switch
          value={formData.consent}
          onValueChange={(newValue) => setFormData.consent(newValue)}
