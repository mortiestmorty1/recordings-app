...
const uploadRecording = async (uri) => {
  ...
  // Adjust the URI for Android file system specifics
  let adjustedUri = uri;
  if (Platform.OS === 'android' && uri.startsWith('file://')) {
      adjustedUri = uri.substring(7); // Removing 'file://' prefix
  }
  ...
  formData.append('audio', {
      uri: Platform.OS === 'android' ? adjustedUri : uri, // Use adjustedUri for Android
      name: fileName,
      type: fileType,
  });
  ...
  const backendUrl = Platform.OS === 'android'
      ? 'http://10.0.2.2:3001/recordings/createRecordings' // Use the special Android localhost alias
      : 'http://localhost:3001/recordings/createRecordings'; // iOS and other platforms can use localhost directly
