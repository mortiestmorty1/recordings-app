...
const uploadRecording = async (uri) => {
    ...
    let fileType = 'audio/mpeg'; // Update this based on the actual file type

    // Prepare the form data
    let formData = new FormData();

    // Append the recording file
    // Make sure the `uri` is correct and accessible
    formData.append('file', {
        uri: Platform.OS === 'ios' ? uri.replace('file://', '') : uri,
        type: fileType,
        name: `recording_${Date.now()}.mp3`, // This should match the server's expectation
    });

    // Append the userId and textId
