...
const RecordingsList = ({ navigation, route }) => {
  ...
  const { email } = route.params.email; // Ensure email is passed as a parameter

  useEffect(() => {
    const fetchRecordings = async () => {
      try {
        const response = await axios.get('http://localhost:3001/getallrecsofuser', {
          params: {
            email: email // Use the email passed to the route
          }
        });
