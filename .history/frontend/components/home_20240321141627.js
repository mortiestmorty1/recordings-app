...
const uploadRecording = async (uri, userId, name, textId, text) => {
  const fileType = 'audio/mp3';
  const timestamp = Date.now();
  const fileName = `recording_${timestamp}_${name}_TextID_${textId}.mp3`;

  let uploadUri = uri;
  // Adjust URI for Android's file system
  if (Platform.OS === 'android' && uri.startsWith('file://')) {
      uploadUri = uri.substring(7); // Remove 'file://' prefix
  }

  const formData = new FormData();
  formData.append('userId', userId);
  formData.append('textId', textId);
  formData.append('textString', text);
  formData.append('audio', {
      // React Native's Blob or File objects could be used here for more complex scenarios
      uri: Platform.OS === 'android' ? uploadUri : uri, // Use adjusted URI for Android
      name: fileName,
      type: fileType,
  });
  ...
  try {
      ...
      if (Platform.OS === 'android') {
          await FileSystem.deleteAsync(`file://${uploadUri}`, { idempotent: true });
      } else {
