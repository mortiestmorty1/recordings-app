...
const Home = ({ navigation }) => {
  ...
  const fetchTexts = async () => {
    try {
      const response = await axios.get('http://yourserver.com/text/getall');
      setTexts(response.data.texts); // Make sure this matches your actual API response structure
    } catch (error) {
  ...
  return (
    <View style={styles.container}>
      {texts.length > 0 && (
        <View>
          <Text>{texts[currentTextIndex]}</Text> {/* Ensure you display the text correctly */}
          <Button
