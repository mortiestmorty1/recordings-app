...
import { View, Text, TextInput, TouchableOpacity, StyleSheet,Switch, ImageBackground, Alert } from 'react-native';
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        const errorResponse = await response.text(); // or response.json() if the server sends JSON response
        throw new Error(errorResponse || 'Something went wrong while registering the user');
      }
  
      // Here we parse the response body as JSON
      const jsonResponse = await response.json();
  
      // Now you can access userId, count, name directly from jsonResponse
