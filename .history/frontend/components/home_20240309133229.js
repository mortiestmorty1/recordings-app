...
import {Image, View, Text, Button, StyleSheet, Alert, Platform ,TouchableWithoutFeedback, Animated} from 'react-native';
...
const Home = ({ navigation, route }) => {
  ...
  const animatedStyle = {
    transform: [
      // Ensure the rotation axis is centered by translating to half the width/height and then translating back after rotation
      { translateX: -50 }, // Half of the SVG width
      { translateY: -50 }, // Half of the SVG height
      {
  ...
  const stopRotation = () => {
    Animated.timing(rotateAnim).stop(); // Stop any running animation
    rotateAnim.setValue(0); // Reset the rotation to 0
  };
  useEffect(() => {
    fetchTexts();
        ...
        <View>
          ...
          >
            <View style={styles.button}>
              ...
              <Animated.View {[styles.overlaySvg1, animatedStyle]}>
