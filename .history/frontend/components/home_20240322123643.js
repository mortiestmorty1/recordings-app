...
const Home = ({ navigation, route }) => {
  ...
  const playRecording = async () => {
    ...
    setPlaybackSound(playbackSound);

    playbackSound.setOnPlaybackStatusUpdate(playbackStatus => {
      if (playbackStatus.didJustFinish) {
        playbackSound.unloadAsync(); // Unload the sound when playback finishes
      }
    });
  };

  // Call this function to play the last recording
  const handlePlayLastRecording = async () => {
    if (lastRecordingUri) {
      await playRecording();
    } else {
      Alert.alert("Playback Error", "There's no recording to play back.");
    }
  };

  // Modify your existing stopRecordingAndUpload function to just stop recording
  // Add a button or call handlePlayLastRecording() when you want to play the recording
  
  // Make sure to unload the playback sound when the component unmounts or when appropriate
  useEffect(() => {
  ...
  return (
   ...
   <View style={styles.actionContainer}>
    ...
    <View style={styles.buttonsContainer}>
      ...
      <TouchableOpacity onPress={handlePlayLastRecording} style={styles.playbackButton}>
        <Text>Play Last Recording</Text>
      </TouchableOpacity>
