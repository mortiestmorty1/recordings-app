...
const uploadRecording = async (uri) => {
    let fileType = 'audio/mpeg'; // or the correct type based on your recording
    let formData = new FormData();
    formData.append('file', {
        uri: uri,
        name: `recording_${Date.now()}.mp3`, // Example file name, adjust as needed
        type: fileType,
    });
    ...
    try {
        ...
        const responseJson = await response.json(); // Try parsing the response as JSON
        console.log('Upload successful:', responseJson);
    } catch (error) {
        console.error('Upload failed:', error);
        if (error.message.includes('Unexpected character')) {
            // This likely means the response is not in JSON format
            Alert.alert("Upload Failed", "The server responded with an unexpected format. Please check the server configuration.");
        } else {
